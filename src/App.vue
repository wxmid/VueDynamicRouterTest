<template>
  <div id="app">
    <!--<aside class="sidbar">
      <ul>
        <li v-for="(route,index) in this.$router.options.routes" v-bind:class="{ active: (isActive == route.name) }" v-bind:isLoginPage="{}"><router-link :to="route.path">{{route.name}}</router-link></li>
      </ul>
    </aside>
